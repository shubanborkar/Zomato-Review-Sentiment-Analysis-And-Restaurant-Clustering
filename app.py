...
@st.cache_data
def load_data(restaurant_file=None, reviews_file=None):
    try:
...
# Load data
restaurant_data, reviews = load_data(restaurant_file, reviews_file)

if restaurant_data is not None and reviews is not None:
    # Preprocess data
    @st.cache_data
    def preprocess_data(restaurant_data, reviews):
        ...
        # Preprocess ratings with robust handling
        reviews["Rating"] = pd.to_numeric(reviews.Rating, errors="coerce")
        numeric_count = reviews["Rating"].notna().sum()
        if numeric_count == 0:
            st.error("No numeric values found in the 'Rating' column. Please ensure the column contains valid numeric ratings.")
            reviews["Rating"] = reviews["Rating"].fillna(0)  # Fallback to 0 if no valid ratings
        else:
    ...
    restaurant_data, reviews, merged_data = preprocess_data(restaurant_data, reviews)
    ...
    with tab1:
        ...
        fig = px.histogram(filtered_restaurants, x='Cost', nbins=30, title="Restaurant Cost Distribution")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab2:
        ...
        if nlp:
            ...
            filtered_reviews['Food_Entities'] = filtered_reviews['Review Cleaned'].apply(extract_food_entities)
    ...
    with tab3:
        ...
        # Clustering
        n_clusters = st.slider("Number of Clusters", 2, 15, 7)
